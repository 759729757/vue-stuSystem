    ...
    var calendarSwitch = (function() {
        ...
        calendarSwitch.prototype = { /*说明：初始化插件*/
            ...
            init: function() {
                ...
                //mask_calendar
                me.mask_calendar=me.selectors.mask_calendar;
                //calendar1
                me.calendar= me.selectors.calendar;
                //startDate
                me.startDate=me.selectors.startDate;
                //endDate1
                me.endDate=me.selectors.endDate;
                //NumDate1
                me.NumDate=me.selectors.NumDate;
                me.index = me.settings.index;
                me.comfire = me.settings.comfireBtn;

                var html = "<div class='headerWrapper'><div class='headerTip'>请选择开始日期</div><div id="+me.comfire.replace('#','')+" class='comfire'>完成</div></div><table class='dateZone'><tr><td class='colo'>日</td><td>一</td><td>二</td><td>三</td><td>四</td><td>五</td><td class='colo'>六</td></tr></table>" + "<div class='tbody'></div>"
                        ...
                        if ($(this).text() == '结束') {
                            ...
                            $(me.sections).find('.tbody .rz').each(function(index, element) {
                                if ($(this).text() == '开始') {
                                    ...
                                    //添加开始到input
                                    $(me.startDate).val(startDayYear + '-' + startDayMonth + '-' + day);
                                }
                                if ($(this).text() == '结束') {
                                    ...
                                    //添加开始到input
                                    $(me.endDate).val(endDayYear + '-' + endDayMonth + '-' + day);
                                   // console.log($("#startDate").val().replace(/[^0-9]/ig, ""))
                                   // console.log($("#endDate").val().replace(/[^0-9]/ig, ""))
                                   // 如果开始等于结束
                                    if (parseInt($(me.startDate).val().replace(/[^0-9]/ig, "")) == parseInt($(me.endDate).val().replace(/[^0-9]/ig, ""))) {
                                        var x = $(me.startDate).val();
                            ...
                            var st = $(me.startDate).val();
                            var en = $(me.endDate).val();
                            //如果开始没值
                            if (st) {
                                me._slider(me.sections)
                                me._callback();
                ...
                $(arry1).on('click', function(index) {
                    ...
                    if($('.calendar').css('display') != "none"){
                        ...
                        if (flag == 0) {
                            ...
                            var st = new Date($(me.startDate).val());
                            var en = new Date($(me.endDate).val());
